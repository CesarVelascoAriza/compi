...
public class menupru extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==menuArcAbr) {    cargarDatos();    }		 			 	 	if(e.getSource()==menuArcSal) {    dispose();        }
		if(e.getSource()==menuArcGua) {   guardarDatos();   }
		if(e.getSource()==menuEdiCop) {    area1.copy();     }
		if(e.getSource()==menuEdiCor) {    area1.cut();      }
		if(e.getSource()==menuEdiPeg) {   area1.paste();    }
		if(e.getSource()==menuEdiSel) {    area1.selectAll(); }
		...
		if(e.getSource()==menuAyuAyu){ayuAyuda();  }
		if(e.getSource()==menuAyuAce){ayuAcerca(); }
		if(e.getSource()==menuEjeCor){ejeCor();    }
	}	
